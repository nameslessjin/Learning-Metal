...
enum DebugCameraFrustum {
    ...
    static func createSphereMesh(camera: Camera) {
        ...
        let mdlMesh = MDLMesh(
            sphereWithExtent: [sphereRadius, sphereRadius, sphereRadius], segments: [10, 10], inwardNormals: false, geometryType: .triangles, allocator: allocator
        )
        let mesh: MTKMesh
        do {
            mesh = try MTKMesh(mesh: mdlMesh, device: Renderer.device)
        } catch {
            fatalError("Failed to create main")
        }
    ...
    static func createMesh(from camera: Camera) {
        ...
        frustumMesh = [
            ...
            farPoints.lowerLeft, nearPoints.lowerLeft, nearPoints.upperRight, farPoints.upperRight,
            farPoints.upperRight, farPoints.lowerRight, farPoints.lowerRight, nearPoints.lowerRight,
            farPoints.upperLeft, farPoints.upperRight, farPoints.lowerLeft, farPoints.lowerRight
        ]
        frustumVertexBuffer = Renderer.device.makeBuffer(bytes: frustumMesh, length:  MemoryLayout<float3>.stride * frustumMesh.count, options: [])
        ...
        forwardVectorBuffer = Renderer.device.makeBuffer(bytes: forwardVectorMesh, length: MemoryLayout<float3>.stride * forwardVectorMesh.count, options: [])
    }
