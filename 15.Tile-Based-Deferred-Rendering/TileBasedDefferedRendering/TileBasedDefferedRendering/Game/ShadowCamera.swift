...
extension Camera {
    ...
    private static func calculatePlanePoints(matrix: float4x4, halfWidth: Float, halfHeight: Float, distance: Float, position: float3) -> FrustumPoints {
        ...
        let upVector: float3 = cross(forwardVector, rightVector)
        ...
        let points = FrustumPoints(
            viewMatrix: matrix, upperLeft: upperLeft, upperRight: upperRight, lowerRight: lowerRight, lowerLeft: lowerLeft
        )
